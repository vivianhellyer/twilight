...
#[derive(Debug)]
pub struct TicketHeaders(Receiver<Option<Headers>>);

impl Future for TicketHeaders {
    type Output = Result<Option<Headers>, Canceled>;
...
#[derive(Debug)]
pub struct TicketReceiver(Receiver<Sender<Option<Headers>>>);

impl Future for TicketReceiver {
    type Output = Result<TicketSender, Canceled>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        self.0.poll_unpin(cx).map_ok(|tx| TicketSender(tx))
    }
}
...
#[derive(Debug)]
pub struct TicketSender(Sender<Option<Headers>>);
...
impl TicketSender {
    ...
    /// occurred.
    pub fn headers(self, headers: Option<Headers>) -> Result<(), Option<Headers>> {
        self.0.send(headers)
    }
}
